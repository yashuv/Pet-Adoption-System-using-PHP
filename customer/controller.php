...
function doInsert(){
	...
	function processwishlist(){
		...
		if(isset($_GET['wishid'])){
		 ...
		 if (isset($res)){
		 		message("Pet has been removed from your wishlist.", "success"); 		 
			redirect(web_root."index.php?q=profile");
		 }
	...
	function addwishlist(){
		...
		if($maxrow>0){
				message("Pet has been already added to your wishlist", "error"); 		 
				redirect(web_root."index.php?q=profile"); 
		}else{
				$query ="INSERT INTO `tblwishlist` (`PROID`, `CUSID`, `WISHDATE`, `WISHSTATS`)  VALUES ('{$proid}','{$id}','".DATE('Y-m-d')."',0)";
				$mydb->setQuery($query);
	 	...
	 	message("Pet has been added to your wishlist", "success"); 		 
			redirect(web_root."index.php?q=profile"); 
		}
